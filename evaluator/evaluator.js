...
function evaluateDutyInstance(policyTriplestore, dutyId, evalConsequences, propertyId, testlogger, evalContext){
    ...
    switch(actionEvalResult){
        ...
        case evalActionExersState[1]:
            ...
            // changed to evaluation in all cases to reflect decision taken by a majority of the POE WG on 2017-10-09
            ...
            return evalDutyState[1]
            /*
            if (!evalConsequences) { // retired on 2017-10-09, see above
                // ... --> break and return Duty Fulfilled
                testlogger.addLine("TESTRESULT: Evaluation of Duty instance '" + dutyId + "', status = " + evalDutyState[1] + " (action not exercised/consequences ignored)")
                return evalDutyState[1]
            }
        ...
        case evalActionExersState[2]:
            // WARNING: only either OPTION 1 or OPTION 2 must be active JS code!!
            ...
            return evalDutyState[3]
            */
            // OPTION 2: return Not-Exercised as a not-existing action can only result in not having been exercised
            // Action is Not-Existing (due to refinements) --> break and return Not-Exercised
            testlogger.addLine("TESTRESULT: Evaluation of Duty instance '" + dutyId + "', status = " + evalDutyState[1] + " (action not existing = not exercised)")
            return evalDutyState[1]
            break;
...
function evaluateActionExercised(policyTriplestore, subjectId, propertyId, testlogger, evalContext){
    ...
    testlogger.addLine("NEXT STEP: Evaluation of ActionExercised of action (of Rule '" + subjectId + "')")
